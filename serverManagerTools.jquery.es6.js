...
exports = function()
{
	...
	let _eventDictionary = new Event('dictionary_setlanguage')
	...
	let _tools = {
		'dictionary': 
		{
			...
			'setLang': (lang) =>
			{
				...
				{
					_lang = lang
					document.dispatchEvent(_eventDictionary)
					return true
				}
				return false
			},
			'get': (term, index, defaultValue) =>
			{
		...
		'showDialog': (message, buttons, template) =>
		{
			...
			return (function (dialogIndex)
			{
				...
				{
					messageText = message.reduce((messageText, message) =>
						{
							return messageText.replace(message[i].key, _tools.dictionary.get(message[i].message, message[i].index))
						},
						_tools.dictionary.get(message[0])
					)
				}
				else
				{
				...
				{
					if (hotkeyList.includes(event.key) && dialogIndex === _dialogIndex)
					{
		...
		'dialog':
		{
			...
			'yesNo': (message, yesCallback, noCallback) =>
			{
				_tools.showDialog(
					message,
					[
			...
			'input': (message, acceptCallback, cancelCallback, defaultText) =>
			{
	...
	function connectWebSocket()
	{
		...
		webSocket.onclose = () =>
		{
			...
			else if (_tools.webSocket.ondisconnect && _tools.webSocket.ondisconnect())
			{
				_webSocket = new connectWebSocket()
			}
		}
